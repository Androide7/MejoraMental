// saveHighScoreVrbs.js
import { ref, push, query, orderByChild, limitToLast, onValue, off } from './firebaseConfig.js';
import { db } from './firebaseConfig.js';

const HIGH_SCORES_PATH = 'globalScores_verbs'; // 📘 Ruta específica para el juego de verbos
...
export function saveHighScore(name, score, id) {
  ...
  push(scoresRef, {
    ...
    console.log('✅ Puntaje guardado en Firebase (verbs)');
  }).catch((error) => {
    console.error('❌ Error al guardar (verbs):', error);
  });
}
...
export function loadHighScores(callback) {
  ...
  onValue(currentListenerRef, (snapshot) => {
    ...
    console.log('📥 Datos recibidos (verbs):', data);
