// renderHighScores.js
import { loadHighScores, saveHighScore } from './saveHighScoreFirebase.js';
...
export function renderHighScores(currentScore) {
  ...
  if (!list || !hsContainer || !message) return;

  const localKey = 'offlineScores';
  let topPosition = -1;
  ...
  if (navigator.onLine) {
    ...
    loadHighScores((scores) => {
      ...
      // ✅ Solo mostrar mensaje si entra al top 3
...
window.addEventListener('online', () => {
  const pendientes = JSON.parse(localStorage.getItem('offlineScores') || '[]');
