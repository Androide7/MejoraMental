...
const HIGH_SCORES_PATH = 'globalScores_memoria'; // 🧠 Ruta específica para el juego de memoria
...
export function saveHighScore(name, score, id) {
  ...
  push(scoresRef, {
    ...
    console.log('✅ Puntaje guardado en Firebase (memoria)');
  }).catch((error) => {
...
export function loadHighScores(callback) {
  ...
  onValue(currentListenerRef, (snapshot) => {
    ...
    console.log('📥 Datos recibidos (memoria):', data);
